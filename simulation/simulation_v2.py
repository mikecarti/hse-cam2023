...
class CenterPlayer(Player):
    ...
    def is_closest_to_ball(self, ball):
        return self == self.team.players[np.argmin([np.linalg.norm(np.array(player.current_position) - np.array(ball)) for player in self.team.players])]

    def get_closest_opposing_player(self, opposing_team):
        return opposing_team.players[np.argmin([np.linalg.norm(np.array(self.current_position) - np.array(pos)) for pos in [player.current_position \
                for player in opposing_team.players]])].current_position
...
class Team:
    def __init__(self, name: str, grid, formation: List[int], opposing_team = None):
        ...
        self.players = [Goalkeeper(name, self, i, pos, grid, [[0, 5], [45, 55]]) \
                if i == 10 else CenterPlayer(name, self, i, pos, grid, [35, 65]) \
                if 4 <= i <= 7 else DefencePlayer(name, self, i, pos, grid, [5, 35]) \
                if 0 <= i <= 3 else OffencePlayer(name, self, i, pos, grid, [65, 95]) for i, pos in enumerate(formation)]
    ...
    def pass_ball(self, ball, from_player_id, to_player_id):
        self.players[from_player_id].hit_ball(ball, self.players[to_player_id].current_position)
