...
with open("cam_control/soccer_config/simulation_config.yaml", "r") as config_file:
...
class Player(Entity):
    ...
    def move(self, target):
        ...
        if self.current_position == self.ball.target and self.ball.controlled_by == None and self.player_id != 10:
            ...
            if self.has_ball_control:
                if self.current_position[0] >= self.team.hit_area[0][0] and self.current_position[0] <= \
                        self.team.hit_area[0][1] \
                        and self.current_position[1] >= self.team.hit_area[1][0] and self.current_position[1] <= \
                        self.team.hit_area[1][1]:
                    self.hit_ball(self.ball,
                                  (self.team.opposing_goals[0], np.random.randint(self.team.opposing_goals[1][0], \
                                                                                  self.team.opposing_goals[1][1])))
                    target = self.get_closest_opposing_player()
                    super().move(target)
                else:
                    dist_player_opp = np.linalg.norm(
                        np.array(self.current_position) - np.array(self.get_closest_opposing_player()))
    ...
    def find_nearest_open_teammate(self):
        open_teammates = [player for player in self.team.players \
                          if self.opponent_between_target(player.current_position,
                                                          self.team.opposing_team) == False and player != self]
        if not open_teammates:
            return -1
        return min(open_teammates, key=lambda player: (player.dist_to_opposing_goals(), \
                                                       np.linalg.norm(np.array(self.current_position) - np.array(
                                                           player.current_position))))

    def get_closest_opposing_player(self):
        return self.team.opposing_team.players[
            np.argmin([np.linalg.norm(np.array(self.current_position) - np.array(pos)) \
                       for pos in
                       [player.current_position for player in self.team.opposing_team.players]])].current_position

    def is_ball_in_zone(self):
        return self.zone[0] <= float(self.ball.current_position[0]) <= self.zone[1]

    def is_closest_to_ball(self):
        return self == self.team.players[
            np.argmin([np.linalg.norm(np.array(player.current_position) - np.array(self.ball.current_position)) \
    ...
    def dist_to_opposing_goals(self):
        return np.linalg.norm(np.array(self.current_position) - np.array([float(self.team.opposing_goals[0]), \
                                                                          float(self.team.opposing_goals[1][1]) - float(
                                                                              self.team.opposing_goals[1][0]) + float(
                                                                              self.team.opposing_goals[1][0])]))
    ...
    def opponent_between_target(self, target, opposing_team):
        for opp_player in opposing_team.players:
            ...
            dist_player_opponent = np.linalg.norm(
                np.array(self.current_position) - np.array(opp_player.current_position))
...
class SoccerMatch:
    def __init__(self, grid, team1, formation1, team2, formation2):
        ...
        self.team1, self.team2 = Team(team1, self.grid, formation1, self.ball), Team(team2, self.grid, formation2,
                                                                                     self.ball)
